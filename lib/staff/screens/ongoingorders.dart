...
//update function
  Future<void> updateorderstate() async {
    try {
      CollectionReference ref =
          FirebaseFirestore.instance.collection(collectionName);
      ref.doc(documentid).update({'orderstatus': 'Completed'});
    } on FirebaseAuthException catch (e) {
      // ignore: avoid_print
    ...
    return SafeArea(
      child: Consumer<CartProvider>(builder: (context, value, _) {
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text("Ongoing $collectionName"), centerTitle: true,
                    ...
                    child: Column(
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Text('The Total Ongoing orders: $documentCount'),
                          ],
                        ),
                      ],
                    ),
  ...
  Future<List<String>> cartOrdersupdate(String email) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      ref1.doc(documentid).update({'orderstatus': 'Completed'});
...
//get customer deviceToken
  ...
  Future<List<String>> getCustomerDeviceTokens(String email) async {
    QuerySnapshot querySnapshot = await firestore
        .collection('users')
        .where('email', isEqualTo: email)
        .get();
...
//send Noification to customer
  Future<void> sendNotificationsToStaff(String email) async {
    // Step 1: Get the device tokens of all staff members
    List<String> customerDeviceTokens = await getCustomerDeviceTokens(email);
            ...
            child: Padding(
              ...
              child: Material(
                ...
                child: MaterialButton(
                  ...
                  onPressed: onpress,
                  child: Text('Send Completed Screen',
